...
AUTOGENERATED_PLAYLIST_DESCRIPTION = "Autogenerada"
...
def get_audio_features_resilient(
    ...
    if os.getenv("SPOTIFY_ENABLE_AUDIO_FEATURES", "").strip().lower() not in ("1", "true", "yes", "y"):
        logger.warning(
            "Audio features esta desactivado por defecto porque Spotify lo marco como legacy/deprecado. "
            "Usa Last.fm/Genius o define SPOTIFY_ENABLE_AUDIO_FEATURES=true bajo tu responsabilidad."
        )
        return {}
        ...
        try:
            ...
            if status == 429:
                ...
                try:
                    ...
                    logger.error(f"Error al obtener artista {artist_id}: {retry_exc}")
            else:
                logger.error(f"Error al obtener artista {artist_id}: {e}")
    return out
...
def ensure_playlist(sp: spotipy.Spotify, user_id: str, name: str, public: bool) -> str:
    ...
    pl = sp.current_user_playlist_create(
        name=name,
        public=public,
        description=AUTOGENERATED_PLAYLIST_DESCRIPTION,
    )
    return pl["id"]


def clear_playlist(sp: spotipy.Spotify, playlist_id: str):
    return sp.playlist_replace_items(playlist_id, [])
