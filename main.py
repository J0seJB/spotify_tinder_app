...
def smart_playlist_mode(sp_user, sp_app, user_id, playlist_name, liked_items,
    ...
    print("-" * 60)
    print("\nBusca canciones semilla. Escribe nombre o artista (vacio para terminar):\n")
    ...
    while True:
        ...
        if not query:
            if not seed_ids:
                print("  Anade al menos 1 cancion semilla.")
                continue
            break
        matches = [
        ...
        for i, (tid, meta) in enumerate(matches):
            marker = " [seleccionada]" if tid in seed_ids else ""
            print(f"  [{i+1}] {meta['name']} - {meta['artists']}{marker}")
        sel = input("  Numero(s) separados por coma: ").strip()
            ...
            if token.isdigit():
                ...
                if 0 <= idx < len(matches):
                    ...
                    if tid not in seed_ids:
                        seed_ids.append(tid)
                        print(f"  OK  {meta['name']} - {meta['artists']}")
        print()

    print(f"\nAnalizando perfil de {len(seed_ids)} cancion(es) semilla...")
    ...
    if genres_sample:
        print(f"   Generos: {', '.join(genres_sample)}")
    if ai_desc:
        print("\nClaude AI:")
        print("-" * 50)
        print(ai_desc)
    ...
    # Las semillas inicializan senales positivas
    ...
    while True:
        ...
        if not pool:
            print("\nNo hay mas canciones para sugerir.")
            break
        ...
        print("  Numeros separados por coma - ENTER si no quieres ninguna:\n")
        for i, (tid, score, meta) in enumerate(batch, 1):
            print(f"  [{i:2d}] {meta.get('name', '?')} - {meta.get('artists', '?')}  [{score:.0%}]")

        print("\n  [0]  Terminar y crear playlist")
        print("  [s]  Saltar estas 10 y ver mas")

        sel = input("\n  Tu seleccion > ").strip().lower()

        if sel == "0":
            break

        if sel in ("s", "skip", ""):
            # Senal de rechazo leve por saltar
            _update_signals(
        ...
        for i, (tid, score, meta) in enumerate(batch):
            if i in selected:
                ...
                print(f"  OK  {meta.get('name')} - {meta.get('artists')}")
            else:
        ...
        cont = input("\n  Ver mas opciones? (s/n): ").strip().lower()
        if cont not in ("s", "si", "yes", "y", ""):
            break
    ...
    if dry_run:
        print("\n[DRY-RUN] No se creara la playlist.")
        return
    ...
    if to_add:
        ...
        print(f"\nOK  {playlist_name} creada con {len(to_add)} canciones en Spotify")
    else:
        print("\nLa playlist ya tenia todas esas canciones.")
    ...
    try:
        ...
        if args.discover_categories:
            ...
            for cat in categories:
                ...
                if to_add:
                    add_tracks_in_chunks(sp_user, pid, to_add)
                    logger.info("OK %s: +%s", playlist_name, len(to_add))
            return
        ...
        if not args.create:
            ...
            return
        if not args.yes:
            confirm = input("Confirmas la creacion de playlists? (s/n): ").strip().lower()
            if confirm not in ("s", "si", "yes", "y"):
                return
        ...
        for cat in CATEGORIES:
            ...
            if to_add:
                add_tracks_in_chunks(sp_user, pid, to_add)
                logger.info(f"OK {playlist_name}: +{len(to_add)}")
