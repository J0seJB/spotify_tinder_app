# views.py — Playlists dinámicas por reglas (versión corregida)
...
def filter_df(df: pd.DataFrame, flt: Dict[str, Any]) -> pd.DataFrame:
    ...
    # Filtro de idioma por heurística de géneros
    lang = flt.get("lang", "").upper()
    latin_pattern = "latin|español|espanol|mexican|argentine|spanish|colombian|reggaeton|cumbia|salsa|bachata"
    if lang == "ES":
...
def main():
    load_dotenv()
    ap = argparse.ArgumentParser(description="Genera playlists dinámicas desde views.json")
    ...
    for v in views:
        ...
        if args.create and sp and uid and len(sub) > 0:
            ...
            print(f"  ✔ Creada: {pl_name} ({len(uris)} canciones)")
