...
def _has_any(genres_set: set[str], needles: List[str]) -> bool:
    # Búsqueda flexible (contiene) sobre el join de géneros
    g = " ".join(genres_set)
    return any(n in g for n in needles)


# ============================================
# Configuración de categorías (12 personalizadas)
# Ajusta pesos, rangos y listas sin tocar lógica
# ============================================

CATEGORY_PROFILES: Dict[str, Dict[str, Any]] = {
    # 1) Chill con amigos (Feid, Bad Bunny, Álvaro Díaz, Latin Mafia, Rels B)
    "Chill con amigos": {
        ...
        "artist_whitelist": ["feid", "bad bunny", "álvaro díaz", "alvaro diaz", "latin mafia", "rels b", "saiko", "mora", "rai na", "micro tdh", "sech"],
    },

    # 2) Rock en español energético
    "Rock ES - energético": {
        ...
        "valence_mu": 0.55,
        "dance_mu": 0.50,
        "genre_needles": ["rock en español", "latin rock", "spanish rock", "argentino rock", "mexican rock"],
        "artist_whitelist": ["enrique bunbury", "hombres g", "maná", "mana", "los enanitos verdes", "la vela puerca", "caramelos de cianuro", "aféctica", "los bunkers"],
    },

    # 3) Rock en español lento
    "Rock ES - lento": {
        ...
        "valence_mu": 0.40,
        "dance_mu": 0.40,
        "genre_needles": ["rock en español", "latin rock", "spanish rock"],
        "artist_whitelist": ["andrés calamaro", "andres calamaro", "fito páez", "fito paez", "soda stereo", "caifanes", "los prisioneros", "enrique bunbury"],
    },
    ...
    # 5) Balada/Pop en español (Andrés Cepeda, Fonseca, etc.)
    "Balada/Pop ES (Cepeda/Fonseca)": {
        ...
        "artist_whitelist": ["andrés cepeda", "andres cepeda", "fonseca", "santiago cruz", "camila", "reik", "morat", "manuel medrano", "ricardo arjona", "pablo alborán", "pablo alboran"],
    },
    ...
    "Techno - lento": {
        ...
        "artist_whitelist": ["artbat", "kölsch", "kolsch", "ben böhmer", "ben boehmer", "maceo plex", "stephan bodzin"],
    },
    ...
    # 8) Depre en español (Kevin Kaarl y similares)
    "Depre ES": {
        ...
        "valence_mu": 0.25,
        "acoustic_mu": 0.50,
        "genre_needles": ["indie en español", "mexican indie", "latin indie", "cantautor", "folk latino", "sad"],
        "artist_whitelist": ["kevin kaarl", "zorra", "ed maverick", "daniel me estas matando", "este man", "silvestre y la naranja"],
    },

    # 9) Depre en inglés (XXXTentacion y afines)
    "Depre EN": {
    ...
    "Rap ES": {
        ...
        "valence_mu": 0.40,
        "dance_mu": 0.55,
        "genre_needles": ["rap espanol", "rap español", "hip hop latino", "latin hip hop", "trap latino", "boom bap español"],
...
# ==================================================
# Scoring principal (features + géneros + artistas)
# ==================================================
...
def score_categories(features: Dict[str, Any], genres: List[str], artists: List[str] | None = None) -> Dict[str, float]:
    ...
    Devuelve {categoria: score 0..1} usando:
      ...
      - Géneros agregados de artistas (strings)
      - Coincidencia de artistas (whitelist)
    Si no hay features, usa heurística con géneros/artistas.
    """
    artists = [a.lower() for a in (artists or [])]
    gset = set((g or "").lower() for g in (genres or []))

    # Si NO hay features útiles, usa sólo géneros/artistas como heurística sencilla
    ...
    for cat, prof in CATEGORY_PROFILES.items():
        ...
        # Bonos por género/artista y modo menor (cuando aplique)
        ...
        if aset and any(a in aset for a in prof.get("artist_whitelist", [])):
            s += 0.08  # pequeño boost adicional por artista
...
# =======================================
# Selección de una o varias categorías
# =======================================
...
def pick_categories_multi(
    ...
    """
    Multi-etiquetado: devuelve varias categorías ordenadas por score.
      - Filtra por umbral (default_min_conf).
      - Usa 'margin_ratio' para incluir categorías cercanas al mejor score.
