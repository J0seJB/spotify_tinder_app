...
export default function PlaylistScreen({ route, navigation }) {
  ...
  async function create() {
    ...
    setError("");
    try {
      const res = await API.createPlaylist(name.trim(), isPublic);
      if (res.ok) {
        setDone(true);
      } else {
        setError(res.detail || "Error creando playlist");
      }
    } catch (e) {
  ...
  if (done) {
    return (
      <LinearGradient colors={["#0a0a0a", "#0a2a15"]} style={styles.center}>
        ...
        <Text style={styles.doneSub}>«{name}» ya está en Spotify</Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <TouchableOpacity style={styles.toggleRow} onPress={() => setIsPublic(!isPublic)}>
        ...
        <Text style={styles.toggleLabel}>{isPublic ? "Pública" : "Privada"}</Text>
      </TouchableOpacity>
