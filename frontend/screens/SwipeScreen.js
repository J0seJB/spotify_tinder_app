...
export default function SwipeScreen({ navigation }) {
  ...
  async function loadNext() {
    ...
    if (res.cards && res.cards.length > 0) {
      ...
      const next = await API.getNext(1);
      if (next.cards && next.cards.length > 0) setNextCard(next.cards[0]);
    } else {
      setCard(null);
    }
    setLoading(false);
  ...
  if (!card) {
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.doneSubtitle}>{approved} canciones aprobadas</Text>
      ...
      <View style={styles.cardArea}>
        ...
        >
          {card.image_url
            ...
            : <View style={[styles.cardImage, styles.noImage]}><Text style={styles.noImageText}>♫</Text></View>
          }
          <LinearGradient colors={["transparent", "rgba(0,0,0,0.95)"]} style={styles.cardGradient}>
            ...
            {false && card.preview_url && (
              <TouchableOpacity style={styles.previewBtn} onPress={togglePreview}>
                <Text style={styles.previewBtnText}>{playing ? "⏸ Pausar" : "▶ Preview"}</Text>
              </TouchableOpacity>
            )}
          </LinearGradient>
...
const styles = StyleSheet.create({
  ...
  headerCount: { color: "#555", fontSize: 14 },
  ...
  noImageText: { fontSize: 80, color: "#333" },
  cardGradient: {
  ...
  previewBtn: {
    marginTop: 12, backgroundColor: "rgba(29,185,84,0.25)",
    borderColor: "#1DB954", borderWidth: 1,
    borderRadius: 20, paddingHorizontal: 16, paddingVertical: 8, alignSelf: "flex-start",
  },
  previewBtnText: { color: "#1DB954", fontWeight: "700", fontSize: 13 },
  badge: {
  ...
  nopeBtnText: { fontSize: 26, color: "#ff4458" },
  likeBtn: { backgroundColor: "#1DB954" },
  likeBtnText: { fontSize: 28, color: "#000" },
  loadingText: { color: "#888", marginTop: 16, fontSize: 15 },
  doneEmoji: { fontSize: 64, marginBottom: 16 },
  doneTitle: { fontSize: 28, fontWeight: "800", color: "#fff" },
  doneSubtitle: { fontSize: 16, color: "#888", marginTop: 8, marginBottom: 32 },
