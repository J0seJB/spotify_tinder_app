...
3. Se complementa con similitud por géneros de Spotify y artistas compartidos.
...
def _tags_to_vector(tags: List[str]) -> List[float]:
    """Convierte tags de Last.fm en vector numérico [energy, valence, dance, acoustic, instrumental]."""
...
def score_candidates(
    ...
    for tid in candidate_ids:
        ...
        # 1. Score Last.fm directo (canción marcada como similar por Last.fm)
        ...
        # 3. Score por géneros de Spotify (ponderado por frecuencia)
        ...
        # Combinar scores según disponibilidad
        if lfm_direct > 0:
            # Last.fm dijo explícitamente que es similar → máxima prioridad
...
def describe_profile_with_ai(
    ...
    prompt = f"""Eres un experto en música y análisis de gustos musicales.

El usuario creó una playlist con estas canciones semilla:
{chr(10).join(seed_names)}

Tags musicales detectados (via Last.fm): {', '.join(top_tags) if top_tags else 'sin datos'}
Géneros detectados (via Spotify): {', '.join(genres_sample) if genres_sample else 'sin datos'}

Canciones sugeridas automáticamente de sus Me Gusta:
{chr(10).join(suggested_names)}

Responde en español con:
1. **Perfil musical** (2-3 oraciones describiendo el vibe/mood)
2. **Por qué estas sugerencias encajan** (1-2 oraciones)
3. **Nombre creativo sugerido para la playlist**

Sé conciso y natural, como un DJ amigo."""
...
def suggest_from_seeds(
    ...
    if lfm_client:
        ...
        logger.info(f"Last.fm encontró {len(lastfm_similar)} coincidencias en tus Me Gusta.")
    else:
    ...
    logger.info(f"Géneros: {genres_str or 'ninguno'}")
    if tags_str:
